...
func setupRealtimeTestEnvironment(t *testing.T) uint64 {
	...
	var err error
	for i := 0; i < 30; i++ {
		blockNumber, err = operations.RealtimeBlockNumber()
		require.NoError(t, err)
		log.Infof("Realtime block number: %d, attempt: %v", blockNumber, i)
		if blockNumber > 0 {
			break
		}
		time.Sleep(1 * time.Second)
...
func deployERC20Contract(t *testing.T, ctx context.Context, privateKey *ecdsa.PrivateKey, client *ethclient.Client, erc20ABI abi.ABI) common.Address {
	publicKey := privateKey.Public()
	...
	log.Infof("ERC20 Contract deployed at: %s, transaction hash: %s", erc20Address.Hex(), tx.Hash().Hex())
	// Wait for contract deployment to be mined
	bind.WaitDeployed(ctx, client, tx)
...
func TestRealtimeRPC(t *testing.T) {
	...
	t.Run("RealtimeGetTransactionByHash", func(t *testing.T) {
		...
		tx, err := operations.RealtimeGetTransactionByHash(common.HexToHash(txHash), &includeExtraInfo)
		require.NoError(t, err)
		log.Infof("RealtimeGetTransactionByHash result type: %T", tx)
	})

	t.Run("RealtimeGetRawTransactionByHash", func(t *testing.T) {
		tx, err := operations.RealtimeGetRawTransactionByHash(common.HexToHash(txHash))
		require.NoError(t, err)
		log.Infof("RealtimeGetRawTransactionByHash result type: %T", tx)
	})

	t.Run("RealtimeGetTransactionReceipt", func(t *testing.T) {
		receipt, err := operations.RealtimeGetTransactionReceipt(common.HexToHash(txHash))
		require.NoError(t, err)
		log.Infof("RealtimeGetTransactionReceipt result type: %T", receipt)
	})

	t.Run("RealtimeGetInternalTransactions", func(t *testing.T) {
		tx, err := operations.RealtimeGetInternalTransactions(common.HexToHash(txHash))
		require.NoError(t, err)
	...
	t.Run("RealtimeGetCode", func(t *testing.T) {
		code, err := operations.RealtimeGetCode(erc20Address)
		require.NoError(t, err)
	...
	t.Run("RealtimeCall", func(t *testing.T) {
		...
		require.NoError(t, err)
		value, err := operations.RealtimeCall(testAddress, erc20Address, "0x100000", "0x1", "0x0", fmt.Sprintf("0x%x", data))
		require.NoError(t, err)
