...
func TestLegacyTx(t *testing.T) {
	...
	assert.Equal(t, emptyMsg.Hash, emptyTx.Hash())
	assert.Equal(t, emptyMsg.From, testFromAddr)
	assert.Equal(t, *emptyMsg.ChainID, *emptyTx.GetChainID())
	assert.Equal(t, emptyMsg.Nonce, emptyTx.GetNonce())
	assert.Equal(t, emptyMsg.Gas, emptyTx.GetGas())
	assert.Equal(t, *emptyMsg.To, testToAddr)
	assert.Equal(t, *emptyMsg.Value, *uint256.NewInt(0))
	assert.Equal(t, hex.EncodeToString(emptyMsg.Data), "")
	v, r, s := emptyTx.RawSignatureValues()
	assert.Equal(t, emptyMsg.R, *r)
	assert.Equal(t, emptyMsg.S, *s)
	assert.Equal(t, emptyMsg.V, *v)
	...
	assert.Equal(t, msg.Hash, rightvrsTx.Hash())
	assert.Equal(t, msg.From, testFromAddr)
	assert.Equal(t, *msg.ChainID, *rightvrsTx.GetChainID())
	assert.Equal(t, msg.Nonce, rightvrsTx.GetNonce())
	assert.Equal(t, msg.Gas, uint64(2000))
	assert.Equal(t, *msg.To, testToAddr)
	assert.Equal(t, *msg.Value, *uint256.NewInt(10))
	assert.Equal(t, hex.EncodeToString(msg.Data), "5544")
	...
	assert.NilError(t, err)
	convertReceipt, err := emptyMsg.GetReceipt()
	assert.NilError(t, err)
	...
	assert.NilError(t, err)
	convertReceipt, err = msg.GetReceipt()
	assert.NilError(t, err)
	assert.Equal(t, convertBlockNumber, blockNumber)
...
func TestAccessListTx(t *testing.T) {
	...
	assert.Equal(t, msg.Hash, signedAccessListTx.Hash())
	assert.Equal(t, msg.From, testFromAddr)
	assert.Equal(t, *msg.ChainID, *signedAccessListTx.GetChainID())
...
func TestDynamicFeeTx(t *testing.T) {
	...
	assert.Equal(t, msg.Hash, signedDynFeeTx.Hash())
	assert.Equal(t, msg.From, testFromAddr)
	assert.Equal(t, *msg.ChainID, *signedDynFeeTx.GetChainID())
...
func TestFromBlobTx(t *testing.T) {
	...
	assert.Equal(t, msg.Hash, blobTx.Hash())
	assert.Equal(t, msg.From, testFromAddr)
	assert.Equal(t, *msg.ChainID, *blobTx.GetChainID())
