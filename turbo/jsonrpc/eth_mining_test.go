...
func TestPendingBlock(t *testing.T) {
	...
	ff := rpchelper.New(ctx, nil, nil, mining, nil, func() {}, m.Log)
	stateCache := kvcache.New(kvcache.DefaultCoherentConfig)
	engine := ethash.NewFaker()
	api := NewEthAPI(NewBaseApi(ff, stateCache, m.BlockReader, nil, false, rpccfg.DefaultEvmCallTimeout, engine,
		m.Dirs), nil, nil, nil, nil, mining, 5000000, 1e18, 100_000, &ethconfig.Defaults, false, 100_000, 128, log.New(), nil, 1000)
	expect := uint64(12345)
