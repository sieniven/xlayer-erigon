...
BRIDGE_VALUE_BIG="100000000000000000000"  # 1 ETH in wei
...
echo "wrappedTokenToTokenInfo"
cast call $BRIDGE_ADDRESS 'function wrappedTokenToTokenInfo(address) public returns (uint32, address)' $L2_WOKB --rpc-url=$L2RPC


# Check balance before bridging
L2_BALANCE_BEFORE_BRIDGE=$(cast call "$L2_WOKB" "balanceOf(address)(uint256)" "$ACCOUNT" --rpc-url "$L2RPC" | awk '{print $1}')
echo "L2_BALANCE_BEFORE_BRIDGE: $L2_BALANCE_BEFORE_BRIDGE"

L1_BALANCE_BEFORE_BRIDGE=$(cast call "$L1_WOKB" "balanceOf(address)(uint256)" "$ACCOUNT" --rpc-url "$L1RPC" | awk '{print $1}')
echo "L1_BALANCE_BEFORE_BRIDGE: $L1_BALANCE_BEFORE_BRIDGE"

# echo "metadata"
# cast call $BRIDGE_ADDRESS 'function getTokenMetadata(address) public returns (bytes)' $L2_WOKB --rpc-url=$L2RPC
...
#     0 $ACCOUNT $BRIDGE_VALUE_BIG $L2_WOKB true 0x
