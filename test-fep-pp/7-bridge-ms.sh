...
cast send \
    ...
    $BRIDGE_ADDRESS \
    'function bridgeMessage(uint32 destinationNetwork, address destinationAddress, bool forceUpdateGlobalExitRoot, bytes permitData) returns()' \
    1 $ACCOUNT true 0x
...
cmd="cast send --legacy --rpc-url $L1RPC --private-key $PRIVATE_KEY $BRIDGE_ADDRESS 'claimMessage(bytes32[32],bytes32[32],uint256,bytes32,bytes32,uint32,address,uint32,address,uint256,bytes)' $MERKLE_PROOF $ROLLUP_MERKLE_PROOF $GLOBAL_INDEX $MER $RER $ORINGIN_NETWORK $ORINGIN_ADDRESS $DESTINATION_NETWORK $ACCOUNT $IN_AMOUNT $METADATA"
